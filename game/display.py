...
class Display:
    ...
    def show_possible_moves_grid(self, internal_grid: List[str]) -> None:
    ...
    def show_game_grid(
        self, internal_grid: List[str]  # This is internal_grid from Grid class
    ) -> None:
    ...
    def _make_grid_string(self, grid_list: List[str]) -> str:
        ...
        for line_num in range(1, 12):
            if line_num in [4, 8]:
                # Rows with horizontal dividors.
                grid += self._display_init[1]

            elif line_num in [2, 6, 10]:
                # Rows where grid can have (X or O).
                ...
                grid += self._display_init[0]
                # Rows with vertical dividors and blank space.
